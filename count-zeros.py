def Count_zeros(arr_A):
    ...
    zeros_arr = []
    #Check1 checks if the parameter passed is an array
    if type(arr_A) is not list:
        raise TypeError("only array allowed")
    #Check2 of checking whether the array is empty
    elif len(arr_A) == 0:
        return "None"
    #Check3: of checking whether 0 is in array
    elif 0 not in arr_A:
        return "No 0's in arr_A"
    #proceeds to counting number of 0's after passing the individual checks
    else:
        for number in arr_A:
            if number == 0:
